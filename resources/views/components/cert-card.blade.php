...
<div class="shadow-xl card card-side bg-base-100">
    <figure>
        <img src="{{ $certificate->cer_image_path }}" class="w-[247px] h-[147px] object-cover" />
    </figure>
    <div class="card-body">
        ...
        <div>
            <div class="badge {{ $certificate->cer_publiced ? 'text-green-600 bg-green-200' : 'text-gray-600 bg-gray-200' }}">
    ...
    <div class="justify-end mr-10">
        <div class="flex items-center justify-center h-full gap-4">
            <form method="POST" action="{{ route('certificates.toggle', $certificate->cer_id) }}">
                @csrf
                @method('PATCH')
                <button type="submit" class="flex items-center justify-center w-10 h-10 border rounded-2xl hover:bg-blue-600 hover:text-white">
            ...
            @if(!$certificate->cer_from_lesson)
                <form method="POST" action="{{ route('certificates.destroy', $certificate->cer_id) }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="flex items-center justify-center w-10 h-10 text-red-600 border rounded-2xl hover:bg-red-600 hover:text-white">
