  ...
  <form method="POST"
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <div class="md:col-span-2">
        ...
        <textarea name="rc_description" rows="5" class="w-full border border-gray-300 textarea textarea-bordered" required>{{ old('rc_description') }}</textarea>
      ...
      <div>
        ...
        <select name="rc_status" class="w-full border border-gray-300 select select-bordered" required>
          @foreach (['open'=>'เปิดรับ','closed'=>'ปิดรับ','draft'=>'ฉบับร่าง'] as $k=>$v)
            <option value="{{ $k }}" @selected(old('rc_status','draft')===$k)>{{ $v }}</option>
          @endforeach
        </select>
