        ...
        <div class="flex flex-col gap-1" x-data="fileUpload()">
            ...
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-500"
                ...
                <p class="text-xs text-gray-400 mt-1">
                    รองรับหลายไฟล์ (PDF, DOCX, PNG, JPG)
                </p>
            </div>
            ...
            <div x-data="{
                    existingFiles: {{ $media->files->map(fn($f) => [
                        'mf_id' => $f->mf_id,
                        'mf_original_name' => $f->mf_original_name,
                        'type' => 'existing',
                        'deleted' => false
                    ])->toJson() }},
                    deletedFiles: [],
                    get allFiles() {
                        return [...this.existingFiles, ...files];
                    },
                ...
                <template x-for="(file, index) in allFiles" :key="file.mf_id ?? index">
                    <div class="flex items-center justify-between p-2 border rounded-lg border-gray-300 bg-white shadow-sm"
                        x-show="!file.deleted">
                        <div class="flex items-center gap-2 flex-1 min-w-0">
                            <i :class="getFileIcon(file.mf_original_name ?? file.name) + ' text-lg flex-shrink-0'"></i>
                            <span x-text="file.mf_original_name ?? file.name" class="truncate"></span>
                        </div>
                        <button type="button"
                                @click="file.type === 'existing' ? (file.deleted = true, deletedFiles.push(file.mf_id)) : removeFile(index - existingFiles.length)"
                                class="ml-2 text-red-500 hover:text-red-700 font-bold flex-shrink-0">&times;</button>

                        {{-- hidden input สำหรับไฟล์เก่าที่ถูกลบ --}}
                        <template x-if="file.type === 'existing' && file.deleted">
                            <input type="hidden" name="delete_files[]" :value="file.mf_id">
                        </template>
                    </div>
                </template>
            </div>
